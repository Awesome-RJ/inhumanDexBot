...
def best_matches(pkmn, data):
    ...
    for key in data:
        for form in data[key]:
            ...
            name = f'{key} {form}' if key not in form else form
            score1 = sum(
                100 / len(name)
            ...
            score_dict[f'{key}/{form}'] = score1 if score1 > score2 else score2
...
def form_name(pkmn, form):
    ...
    if pkmn in ['Ho Oh', 'Jangmo O', 'Hakamoo O', 'Kommo O']:
        ...
        pkmn = 'Nidoran♂'
    return form if pkmn in form else f'{pkmn} ({form})'
...
def get_base_data(pkmn, pkmn_name):
    ...
    typee = ''.join(f' / {i}' for i in pkmn['type'].values())
    typee = typee[3:]
...
def get_advanced_data(pkmn):
    ...
    egg_groups = ''.join(f' / {i}' for i in pkmn['egg_groups'])
    ...
    name_origin = ''.join(f', {i} ({j})' for i, j in pkmn['name_origin'].items())
    ...
    for move, info in zip(move_list, info_list):
        ...
        if index >= minn and index <= maxx:
            if type(info['method']) == list:
                method = ''.join(f' / {i}' for i in info['method'])
                method = method[3:]
            else:
                method = info['method']
            ...
            if location in locations:
                ...
                for game2, location2 in zip(games, locations):
                    if location == location2:
                        games[games.index(game2)] += f'/{game}'
            else:
                # Initialize lists
                games.append(game)
...
def set_page_buttons(page, pages, *args):
    try:
        ...
        form = args[1]
        callback_data_list = [
            f'moveset/{pkmn}/{form}/1',
            f'moveset/{pkmn}' + '/' + form + '/' + str(page - 1),
            f'moveset/{pkmn}/' + form + '/' + str(page),
            f'moveset/{pkmn}/{form}/{str(page+1)}',
            f'moveset/{pkmn}/{form}/' + str(pages),
            f'basic_infos/{pkmn}/' + form,
        ]
    ...
    page_button = InlineKeyboardButton(
        text=f'•{str(page)}•', callback_data=callback_data_list[2]
    )
    ...
    suc = InlineKeyboardButton(
        ...
        text=f'{str(pages)}>>', callback_data=callback_data_list[4]
    )
